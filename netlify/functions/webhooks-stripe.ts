...
export default async function handler(req: Request) {
  ...
  if (!stripeKey) return new Response(JSON.stringify({ error: 'Missing STRIPE_SECRET_KEY' }), { status: 500 });
  if (!webhookSecret) return new Response(JSON.stringify({ error: 'Missing STRIPE_WEBHOOK_SECRET' }), { status: 500 });
  if (!supabaseUrl) return new Response(JSON.stringify({ error: 'Missing SUPABASE_URL' }), { status: 500 });
  if (!supabaseServiceKey) return new Response(JSON.stringify({ error: 'Missing SUPABASE_SERVICE_ROLE_KEY' }), { status: 500 });
